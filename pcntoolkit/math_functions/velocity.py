...
def fisher_transform(cor: np.ndarray) -> np.ndarray:
    """Fisher z-transform of a correlation, with clipping for numerical safety."""
    epsilon = 1e-13
...
def offset_indices(max_age: int, bandwidth: int):
    """Yield upper-triangular (age1, age2) index pairs within ``bandwidth``.
...
def design_matrix(bandwidth: int, Sigma: np.ndarray) -> pd.DataFrame:
    """Construct the regression design matrix used to fill missing correlations.
    ...
    Sigma : np.ndarray
        Correlation matrix (possibly with missing values) for a single response
        variable. The 0'th column represents an age of 0.

    Returns
    -------
    pd.DataFrame
        Design matrix of regressors and a (possibly NaN) target column ``y``.
    """
...
def get_correlation_matrix(data: "NormData", bandwidth: int, covariate_name: str = "age") -> xr.DataArray:
    ...
    if not df["subject_ids"].duplicated().any():
        raise ValueError(
            ...
            "A z-score correlation matrix requires longitudinal data (multiple "
            "observations per subject_id at different ages)."
        )
    ...
    # fill_missing only populates off-diagonal (offset >= 1) entries, so restore
    # the diagonal: a subject's z-score is perfectly correlated with itself.
