...
from pcntoolkit.math_functions.velocity import compute_thrivelines, get_thrive_lines
...
def test_get_thrive_lines_requires_hop_dimension():
    with pytest.raises(ValueError, match="hop"):
        get_thrive_lines(xr.DataArray([0.8]), start_z=0.0)


def test_get_thrive_lines_bayer_update():
    hop = xr.DataArray([0.8], dims=("hop",), coords={"hop": [0]}, attrs={"timepoint_diff": 2})
    z_path = get_thrive_lines(hop, start_z=1.0, z_thrive=-1.96)
